// Arquivo: frontend/src/app/select-profile/page.tsx
import Image from 'next/image';
import Link from 'next/link';

// Componente reutilizável para os cards de perfil
const ProfileCard = ({ href, iconSrc, title, description }: {
  href: string;
  iconSrc: string;
  title: string;
  description: string;
  ...
  <Link href={href}>
    <div className="flex flex-col items-center p-6 bg-white border-2 border-gray-200 rounded-xl shadow-md transition-all duration-300 hover:shadow-xl hover:border-blue-500 hover:scale-105 cursor-pointer">
  ...
  return (
    <main className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl">
        ...
        />
        <ProfileCard
          href="/enter-id?profile=other"
          iconSrc="/icons/other-icon.png"
          title="Outro"
          description="Acesso para pais, suporte técnico e outros perfis de visitante."
        />
      </div>
    </main>
  );
